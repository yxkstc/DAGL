...
public class BaseDao {
     ...
     * @param paramsValue sql语句中占位符对应的值(如果没有占位符，传入null)
     */
    public void update(String sql, byte[] paramsValue) {
        try {
            // update-1获取连接
            con = JDBCUtils.getConnection();
            // update-2创建执行命令的stmt对象
     ...
     * 查询附件名称
     */
    public String queryEnclosureName(String sql, String EnclosureID) {
        ...
        try {
            ...
            // query-2. 创建stmt对象
            pstmt = con.prepareStatement(sql);
            pstmt.setObject(1, EnclosureID);
            // query-3. 执行查询
            rs = pstmt.executeQuery();
            // query-4. 获取结果集元数据
            while (rs.next()) {
                EnclosureName = rs.getString(1);
            }
     ...
     * 查询附件内容
     */
    public byte[] queryEnclosure(String sql, String EnclosureID) {
        byte[] Enclosure = null;
        try {
            // query-1. 获取连接
            con = JDBCUtils.getConnection();
